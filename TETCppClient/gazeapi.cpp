...
namespace gtl
{
    ...
    public:
        ...
        // GazeApi support
        bool connect(bool push_mode, const char* hostname, std::string const & port)
        {
            ...
            bool const success = m_socket.connect(hostname, m_port);
    ...
    {
        return connect(push_mode, "127.0.0.1", 6555);
    }
    
    bool GazeApi::connect(bool push_mode, const char* hostname, unsigned short port)
    {
        std::stringstream ss;
        ss << port;
        return m_engine->connect(push_mode, hostname, ss.str());
    }
